...
以前の「1ネタ×16タイプ」形式（テーマ別カルーセル）は生成を止めた。未送信だった223本は
**同じコピーのまま新しいレイアウトで描き直して**あり（`scripts/convert-legacy-stock.py`）、
新形式の在庫が切れた日の穴埋めとして送られる（「投稿順」参照）。旧レイアウトの画像はもう送らない。
...
描き直した在庫（穴埋め）も同じ経路を通る。番号だけ `L0001-manual` のようにL系列になっている。
...
**シリーズ**は1つのお題を16タイプぶん並べたもの（取扱説明書・相性診断）。**01から16まで連続で出し、
途中に別形式を挟まない。** 同じシリーズは同じルック・同じ配色・同じレイアウトで描く。
**単発**は1投稿で16タイプを扱うもの（16タイプ一覧・ランキング）。

順番は「シリーズ16本 → 単発1本 → 次のシリーズ16本 → 単発1本」。1日10本なのでシリーズは1日半で終わり、
その間の単発が切り替えになる。
...
シリーズの角度（基本/恋愛/友達/仕事）は、その形式が一巡するたびに次へ進む。
...
進み具合は `state/format_state.json`（次の番号・進行中のシリーズと位置・形式ごとのカーソル・追加されたお題）。
`sync` がこれもコミットする。
...
| `PUBLISH_FILLER` | 穴埋めを何本ぶん `docs/media` に上げておくか（30） |
...
- **生成**は前日分まで（`MAX_BACKLOG_DAYS = 1`）。それ以前の日は作り直さない。数日止まった分は穴埋めの在庫（223本＝22日分）が埋める
...
旧レイアウトの残骸（`delivery/phone/<ネタ>/` と `out/<ネタ>/post_*/slides/`）は、描き直した今はもう
送りません。`cleanup` は従来どおり、**テーマの全投稿が送信済み**で**最後の送信から60日経った**テーマについて:
...
キューは1本道です。**その日書いたものを作った順に送り、穴埋め（L系列）はその後ろ。**

1日10本書いて10本送るので、穴埋めが出るのは「書けなかった日」——PCが落ちていた、APIが落ちていた——
だけです。そのために置いてあります。

穴埋めは223本ありますが、**公開するのは先頭30本だけ**（`PUBLISH_FILLER`）。
全部を `docs/media` に上げるとJPEGが250MB増えて、その大半は一度も使われないためです。
キューが進めば次の30本が自動で上がります。

送信が弾かれた本（受信箱が満杯など）はキューの先頭に残り、次のスロットで同じ本から再開します。
...
ルックと配色は**シリーズ単位**で決まる。投稿ごとに切り替えると、同じお題の16本が16通りの
デザインに見えてしまうため。レイアウト（要素の並び）は形式が同じなら常に同じ。
...
キャラに渡す。長い見出しが顔にかぶらないのはこのため。本文スライドは本文量にかかわらず常に
同じ配置（左2/3がコピー、右の列にキャラ）にしてある。以前は本文が短いときだけキャラを上に置いていて、
同じシリーズの中で配置が変わっていた。

### 配色は測って決める

文字と地のコントラスト比（WCAG）で選ぶ。本文は4.5:1、大きな文字は3:1が下限。

**地色は中間の明度に置かない。** パレットのアクセントをそのまま地に敷くと、黒文字で4:1、白文字でも4:1にしかならず、
どちらも読みにくい。キャラをその2色の間に写像すると灰色に濁る。
[contrast.py](src/mbti_tiktok_bot/design/contrast.py) が色相を保ったまま濃い側か淡い側へ寄せ、
文字色はコントラストの高い方を選ぶ。

`tests/generator/test_palettes.py` が 4ルック × 24パレット × 両モードの**1,296組**すべてで4.5:1以上を課している。
...
未送信だった223本は、コピーを1文字も書き換えずに新レイアウトへ移した。
旧形式の1投稿は「1タイプ＋フック＋5〜8個の見出し／本文」で、これは `section` レイアウトと同じ形。

```bash
.venv-linux/bin/python scripts/convert-legacy-stock.py --dry-run
.venv-linux/bin/python scripts/convert-legacy-stock.py --limit 10
```

移すときに、旧描画が落としていたものが2つ戻る。**フック**（全投稿に書かれていたのに一度も描画
されていなかった）と、**特性のチップ**（`場を明るくする / 愛嬌がある` と本文に流し込まれていた205か所を、
チップの並びに戻した）。

### 旧形式の描画

以下は旧形式（テーマ別カルーセル）の描画の説明。生成も送信も止めているが、コードは残っている。
