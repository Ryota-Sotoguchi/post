...
**テーマは8本以上描けてから公開されます。** 1テーマ = MBTI 16タイプぶんですが、
実際には数日かけて描き足されるため、描きかけを公開すると数本だけがキューに入って
しまいます。8本未満のテーマは `Holding back 「テーマ名」: 3/8 drawn` と表示して
見送り、次の sync で条件を満たしたときに公開します。

**タスクを登録しないと sync は回りません。** 2026-09-02 に一度手動で実行した
きり登録されておらず、マニフェストが1週間凍結して、描き足された12本が
キューに入らないまま次のテーマへ進んでしまったことがあります。登録できたかは
`Get-ScheduledTask -TaskName 'TikTok Image Import'` で、動いたかは
`logs/sync.log` と `status` の `Manifest` 行（2日以上古いと警告）で確認します。
