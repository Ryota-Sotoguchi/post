...
class ConfigTests(unittest.TestCase):
    ...
    def test_load_config_prefers_root_env_over_venv_env_file(self) -> None:
        # The root .env is shared with the poster half. .venv/.env is still read
        # so an existing checkout keeps working, but it only fills in keys the
        # root file leaves unset.
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
        ...
        self.assertFalse(config.phone_export_auto)
        self.assertEqual(config.official_images_dir, root / "root_images")
