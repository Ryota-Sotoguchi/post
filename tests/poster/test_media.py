...
from tiktok_poster.catalog import scan_fresh
...
def test_slides_are_written_as_jpeg(config: Config) -> None:
    ...
    post = scan_fresh(config.source_dir)[0]
...
def test_oversized_slides_are_capped_at_1080(config: Config, tmp_path: Path) -> None:
    post_dir = config.source_dir / "_posts" / "00001-gallery"
    Image.new("RGB", (2160, 3840), (10, 10, 10)).save(post_dir / "slide_01.png")

    post = next(p for p in scan_fresh(config.source_dir) if p.source_dir == post_dir)
    written = publish_post(config, post)
    with Image.open(written[0]) as image:
...
def test_public_urls_follow_the_verified_prefix(config: Config) -> None:
    ...
    assert len(urls) == 7
    assert all(url.startswith(f"{config.pages_base_url}/{post.key}/") for url in urls)
    assert urls[0].endswith("/01.jpg")


def test_prune_keeps_only_the_named_carousels(config: Config) -> None:
    posts = scan_fresh(config.source_dir)[:3]
    for post in posts:
        publish_post(config, post)
...
def test_prune_clears_the_folder_it_empties(config: Config) -> None:
    post = scan_fresh(config.source_dir)[0]
    publish_post(config, post)

    prune(config, set())

    assert not (config.publish_dir / "posts").exists()
